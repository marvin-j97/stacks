...
fn handle_clipboard_update(state: &mut state::State, line: &str, app: &tauri::AppHandle) {
    ...
    let packet = if let Some(content) = clipped["text"].as_str() {
        Some(state
            .store
            .add(content.as_bytes(), MimeType::TextPlain, curr_stack))
    }
    else if let Some(content) = clipped["image"].as_str() {
        let content = content.strip_prefix("data:image/png;base64,").unwrap();
        let content = util::b64decode(content);
